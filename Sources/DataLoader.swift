...
public final class DataLoader: DataLoading {
    ...
    public var observer: DataLoaderObserving? {
        get { return impl.observer }
        set { impl.observer = newValue }
    }
...
private final class _DataLoader: NSObject, URLSessionDataDelegate {
    ...
    func loadData(with request: URLRequest,
        ...
        task.resume()
        observer?.dataLoader(session: session, didResumeDataTask: task)
        return task
    }
    ...
    func urlSession(_ session: URLSession,
        ...
        observer?.dataLoader(session: session, dataTask: dataTask, didReceive: response)
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        observer?.dataLoader(session: session, task: task, didCompleteWithError: error)
    ...
    func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        observer?.dataLoader(session: session, dataTask: dataTask, didReceive: data)
...
/// Allows you to tap into internal events handled by the data loader.
/// Events are delivered on the internal data loader queue.
public protocol DataLoaderObserving {
    func dataLoader(session: URLSession, didResumeDataTask: URLSessionDataTask)
    func dataLoader(session: URLSession, dataTask: URLSessionDataTask, didReceive response: URLResponse)
    func dataLoader(session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?)
    func dataLoader(session: URLSession, dataTask: URLSessionDataTask, didReceive data: Data)
}

public extension DataLoaderObserving {
    func dataLoader(session: URLSession, didResumeDataTask: URLSessionDataTask) {}
    func dataLoader(session: URLSession, dataTask: URLSessionDataTask, didReceive response: URLResponse) {}
    func dataLoader(session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {}
    func dataLoader(session: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {}
